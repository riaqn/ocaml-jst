...
let transl_alloc_mode alloc_mode =
  match Mode.Alloc.constrain_default alloc_mode with
  | {locality=Global; uniqueness=Unique;_} -> alloc_heap_unique
  | {locality=Local; uniqueness=Unique;_} -> alloc_local_unique
  | {locality=Global; uniqueness=Shared;_} -> alloc_heap_shared
  | {locality=Local; uniqueness=Shared;_} -> alloc_local_shared

let transl_exp_mode e =
  let alloc_mode = Mode.Value.regional_to_global_alloc e.exp_mode in
...
let rec push_defaults loc bindings use_lhs cases partial warnings =
  ...
  | [{c_lhs=pat; c_guard=None;
      c_rhs={exp_attributes=[{Parsetree.attr_name = {txt="#default"};_}];
             exp_desc = Texp_let
               (Nonrecursive, binds,
                ({exp_desc = Texp_function _} as e2), bctx)}}] ->
      push_defaults loc (Bind_value (binds, bctx) :: bindings) true
...
and transl_exp0 ~in_new_scope ~scopes e =
  ...
  | Texp_send(expr, met, pos) ->
      let lam =
        ...
        | Tmeth_val id ->
            ...
            Lsend (Self, Lvar id, obj, [], pos, lmode, loc)
        | Tmeth_name nm ->
            let obj = transl_exp ~scopes expr in
  ...
  | Texp_lazy e ->
      ...
      | `Other ->
         ...
         in
          Lprim(Pmakeblock(Config.lazy_tag, Mutable, None, alloc_heap_shared), [fn],
                of_location ~scopes e.exp_loc)
      end
...
and transl_tupled_function
  ...
  let partial_mode =
    ...
    | More_args {partial_mode} | Final_arg {partial_mode} ->
      fst (transl_alloc_mode partial_mode)
  in
...
and transl_letop ~scopes loc env let_ ands param case partial warnings =
  ...
  let func =
    ...
    let curry = More_args { partial_mode = Mode.Alloc.default } in
