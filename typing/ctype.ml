  ...
  let rec go ty =
    if TypeSet.mem ty !visited then () else begin
      ...
      | Tarrow ((_,marg,mret),targ,tret,_) ->
         let _ = Mode.Alloc.constrain_default marg in
         let _ = Mode.Alloc.constrain_default mret in
         go targ; go tret
...
let prim_mode mvar = function
  | Primitive.Prim_global, _ -> Mode.Alloc.default
...
let instance_prim_mode (desc : Primitive.description) ty =
  ...
  if is_poly desc.prim_native_repr_res ||
    ...
    instance_prim_locals desc.prim_native_repr_args
      mode Mode.Alloc.default finalret ty,
    Some mode
  else
    ty, None
