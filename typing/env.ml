          ...
          SigL_value(id, decl, _) ->
            ...
            let vda =
              { vda_description = decl'; vda_address = addr;
                vda_mode = Mode.Value.global; vda_shape }
            in
...
let add_value ?check ?shape ?(mode = Mode.Value.global) id desc env =
...
let enter_value ?check name desc env =
  ...
  let addr = value_declaration_address env id desc in
  let env = store_value ?check Mode.Value.global id addr desc (Shape.leaf desc.val_uid) env in
  (id, env)
