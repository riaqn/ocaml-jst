...
module Solver (L : Lattice) = struct
  ...
  let print ?(verbose = true) ?label ppf = function
    ...
    | Amodevar v -> 
      ...
      if verbose then
        ...
        Format.fprintf ppf "<modevar>"
end

module Locality = struct
  module T = struct
    include Modes.Locality
  ...
  let constrain_default = constrain_lower
...
module Uniqueness = struct
  ...
  let constrain_default = constrain_upper
...
module Linearity = struct
  ...
  let join ms =
    let rec aux vars = function
      | [] -> joinvars vars
      | Amode Many :: ms -> aux vars ms
      | Amode Once :: _ -> Amode Once
      | Amodevar v :: ms -> aux (v :: vars) ms
    in aux [] ms
end
...
module Alloc = struct
  ...
  let local = {default with locality = Locality.local}
  let unique = {default with uniqueness = Uniqueness.unique}
  ...
  (* constrain to the default modes*)
  let constrain_default {locality; uniqueness; linearity} =
    {locality=Locality.constrain_default locality 
    ;uniqueness=Uniqueness.constrain_default uniqueness 
    ;linearity=Linearity.constrain_default linearity 
    }
  ...
  let of_linearity linearity = 
    { locality = Locality.newvar (); uniqueness = Uniqueness.newvar () ; linearity}
...
module Value = struct
  ...
  let r_as_l : const -> Alloc.const = function
    | {locality; uniqueness; linearity} -> {locality=Regionality.r_as_l locality; uniqueness; linearity}
    
  [@@warning "-unused-value-declaration"]

  let r_as_g : const -> Alloc.const = function
  | {locality; uniqueness; linearity} -> {locality=Regionality.r_as_g locality; uniqueness; linearity}
  ...
  let default =
    {locality = Regionality.default
    ;uniqueness = Uniqueness.default
    ;linearity = Linearity.default
    }

  let regional = {default with locality = Regionality.regional}

  let local = {default with locality = Regionality.local}
