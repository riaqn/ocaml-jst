...
let join_modes  {locality = lm1;
  ...
  ;uniqueness = join_uniqueness um1 um2
  ;linearity = join_linearity linm1 linm2
  }
...
and print_out_type_mode mode ppf ty =
  ...
  let is_once = match mode.linearity with
    ...
    | _ -> false
  in  
  if is_local || is_unique || is_once then
    if (not is_local || Clflags.Extension.is_enabled Local)
    && (not is_unique || Clflags.Extension.is_enabled Unique) 
    && (not is_once || Clflags.Extension.is_enabled Unique)
    then begin
