...
let extra_pat =
  make_pat
    (Tpat_var (Ident.create_local "+", mknoloc "+", Mode.Value.disallow_right (Mode.Value.max Mode.Regionality.Index.innermost)))
      ...
      | Construct { cstr_tag = Extension _ } ->
          ...
          make_pat
            (Tpat_var (Ident.create_local "*extension*",
                       ...
                       Mode.Value.disallow_right (Mode.Value.max Mode.Regionality.Index.innermost)))
            Ctype.none Env.empty
      | Construct _ ->
