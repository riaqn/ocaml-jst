...
let rec constructor_type constr cty =
  ...
  | Cty_signature _ ->
      constr
  | Cty_arrow (l, ty, cty) ->
      let arrow_desc = l, Mode.Alloc.default, Mode.Alloc.default in
...
let type_constraint val_env sty sty' loc =
  let cty  = transl_simple_type val_env ~fixed:false Global Many sty in
...
let rec class_type_field env sign self_scope ctf =
  ...
  | Pctf_val ({txt=lab}, mut, virt, sty) ->
      mkctf_with_attrs
        (fun () ->
          let cty = transl_simple_type env ~fixed:false Global Many sty in
          let ty = cty.ctyp_type in
...
and class_signature virt env pcsig self_scope loc =
  ...
  let self_cty = transl_simple_type env ~fixed:false Global Many sty in
  ...
  | Pcf_val (label, mut, Cfk_virtual styp) ->
      with_attrs
        (fun () ->
           ...
           let cty = Typetexp.transl_simple_type val_env ~fixed:false Global Many styp in
           let ty = cty.ctyp_type in
        ...
        (fun () ->
           ...
           let ty =
             ...
             | Some sty ->
                 let sty = Ast_helper.Typ.force_poly sty in
                 let cty' =
                   Typetexp.transl_simple_type val_env ~fixed:false Global Many sty
                 in
                 cty'.ctyp_type
           in
...
and class_expr_aux cl_num val_env met_env virt self_scope scl =
  ...
  | Pcl_constr (lid, styl) ->
      ...
      let tyl = List.map
          (fun sty -> transl_simple_type val_env ~fixed:false Global Many sty)
          styl
      in
      let (params, clty) =
