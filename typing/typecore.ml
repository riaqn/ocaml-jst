    ...
    List.fold_left
      (fun (ty_ret, mode_ret, closed_args) (lbl, arg) ->
         ...
         | Omitted { mode_fun; ty_arg; mode_arg; mode_arr; level } ->
             ...
             let ty_ret =
               newty2 ~level
                 (Tarrow ((lbl, mode_arg, mode_arr', mode_ret), ty_arg, ty_ret, commu_ok))
             in
             let mode_ret =
...
let rec approx_type env sty =
  match sty.ptyp_desc with
    Ptyp_arrow (p, _, _, sty) ->
      ...
      newty (Tarrow ((p,marg,marr,mret), ty1, ret, commu_ok))
  | Ptyp_tuple args ->
...
let rec type_approx env sexp =
  ...
  | Pexp_fun (p, _, spat, e) ->
      ...
      newty (Tarrow((p,marg,marr,mret), ty, ret, commu_ok))
  | Pexp_function ({pc_rhs=e}::_) ->
      let ret = type_approx env e in
      let marg = Mode.Alloc.newvar () in
      let marr = Mode.Uniqueness.newvar () in
      let mret = Mode.Alloc.newvar () in
      newty (Tarrow((Nolabel,marg,marr,mret), newvar (), ret, commu_ok))
...
let check_apply_prim_type prim typ =
  ...
  | Tarrow ((Nolabel,_,_,_),a,b,_) ->
      begin match get_desc b with
      | Tarrow((Nolabel,_,_,_),c,d,_) ->
          let f, x, res =
            match prim with
            | Apply -> a, c, d
            | Revapply -> c, a, d
        ...
        (path_self, _) ->
          let type_override (lab, snewval) =
            begin try
              ...
              (id, lab, type_expect env mode_global_shared snewval (mk_expected (instance ty)))
            with
              Not_found ->
...
and type_function ?in_function
  ...
  re {
    ...
    exp_loc = loc; exp_extra = [];
    exp_type =
      instance (newgenty (Tarrow((arg_label,arg_mode,arr_mode,ret_mode),
...
and type_argument ?explanation ?recarg env (mode : expected_mode) sarg
  ...
  let rec loosen_ret_modes ty' ty =
    ...
    | Tarrow((l, marg, marr, mret), ty_arg', ty_res', _),
      ...
      if changed || changed' then
        newty2 ~level:lv' (Tarrow((l, marg, marr, mret), ty_arg', ty_res', commu_ok)),
        newty2 ~level:lv  (Tarrow((l, marg, marr, mret), ty_arg,  ty_res,  commu_ok)),
        true
      else
        ty', ty, false
    ...
    Some (safe_expect, lv) ->
      ...
      let marg, ty_arg, mret, ty_res =
        match get_desc (expand_head env ty_expected') with
          Tarrow((Nolabel,marg,_,mret),ty_arg,ty_res,_) ->
