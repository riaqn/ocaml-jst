...
let mode_local expected_mode =
  { position = Nontail;
    escaping_context = None;
    shared_context = [];
    mode = Mode.Value.to_local expected_mode.mode;
    tuple_modes = [] }
...
let mode_global expected_mode =
  { position = Nontail;
    escaping_context = None;
    shared_context = [];
    mode = Mode.Value.to_global expected_mode.mode;
    tuple_modes = [] }
...
let mode_once expected_mode =
  { position = Nontail;
    escaping_context = None;
    shared_context = [];
    mode = Mode.Value.with_linearity Mode.Linearity.once expected_mode.mode;
    tuple_modes = [] }
...
let escape ~loc ~env ~reason m =
  match Mode.Value.equate m Mode.Value.default with
  | Ok () -> ()
  | Error e -> raise (Error(loc, env, Submode_failed(e, reason, None, [])))
...
let mode_cross env (ty : type_expr) mode =
  if is_principal ty then begin
    ...
    | Type_immediacy.Always -> Mode.Value.newvar ()
    | Type_immediacy.Always_on_64bits when Sys.word_size = 64 ->
        Mode.Value.newvar ()  (* floating and relaxed *)
    | _ -> mode
  end
  else mode
...
and type_pat_aux
  ...
  let type_pat_array mutability spl =
    ...
    let ty_elt = solve_Ppat_array ~refine loc env mutability expected_ty in
      map_fold_cont (fun p -> type_pat ~alloc_mode:(simple_pat_mode Mode.Value.default)
  ...
  | Ppat_construct(lid, sarg) ->
      ...
      map_fold_cont
        (fun (p,(ty, gf)) ->
           let alloc_mode =
             match gf with
             | Global -> Mode.Value.to_global alloc_mode.mode
  ...
  | Ppat_exception p ->
      let alloc_mode = simple_pat_mode (Mode.Value.default) in
  ...
  | Ptyp_arrow (p, ({ ptyp_desc = Ptyp_poly _ } as arg_sty), sty) ->
      if is_optional p then newvar ()
        ...
        let arg_ty =
          ...
          Typetexp.transl_simple_type env ~fixed:false arg_mode.locality arg_mode.linearity arg_sty
        in
...
let type_pattern_approx env spat ty_expected =
  ...
  | Ppat_constraint(_, ({ptyp_desc=Ptyp_poly _} as sty)) ->
      ...
      let ty_pat =
        Typetexp.transl_simple_type env ~fixed:false locality linearity sty
      in
...
let maybe_force_shared ~loc ~env uniqueness linearity =
  if not (Clflags.Extension.is_enabled Unique) then begin
  ...
  | Ok () -> begin match Mode.Uniqueness.submode (Mode.Uniqueness.shared) uniqueness with
            | Ok () -> ()
...
and type_expect_
  ...
  | Pexp_ident lid ->
      ...
      let exp_desc =
        ...
        | Val_self (_, _, _, cl_num) ->
            ...
            in
            maybe_force_shared ~env ~loc mode.uniqueness mode.linearity;
  ...
  | Pexp_function caselist ->
      type_function ?in_function
        ...
        ty_expected_explained Nolabel ~amodes:(Mode.Alloc.newvar ()) ~has_poly:false caselist
  | Pexp_apply
    ({ pexp_desc = Pexp_extension({txt = "extension.once"}, PStr []) },
    [Nolabel, sbody]) ->
  ...
  | Pexp_apply
    ({ pexp_desc = Pexp_extension({txt = ("ocaml.once" | "once")}, PStr []) },
      [Nolabel, sbody]) ->
    let mode = mode_once expected_mode in
    submode ~loc ~env mode.mode expected_mode;
    type_expect ?in_function ~recarg env mode sbody ty_expected_explained
  ...
  | Pexp_apply
      ...
      if not (Clflags.Extension.is_enabled Local) then
        raise (Typetexp.Error (loc, Env.empty, Unsupported_extension Local));
      let mode = expect_mode_cross env ty_expected (mode_local expected_mode) in
  ...
  | Pexp_apply
      ({ pexp_desc = Pexp_extension({txt = "extension.escape"}, PStr []) },
       [Nolabel, sbody]) ->
      type_expect ?in_function ~recarg env (mode_global expected_mode) sbody
        ty_expected_explained
  | Pexp_apply(sfunct, sargs) ->
      ...
      rue {
        exp_desc = Texp_apply(funct, args, position,
          Mode.Value.regional_to_global_alloc expected_mode.mode);
        exp_loc = loc; exp_extra = [];
        exp_type = ty_res;
        exp_attributes = sexp.pexp_attributes;
        exp_env = env }
  ...
  | Pexp_try(sbody, caselist) ->
      ...
      in
      let arg_mode = simple_pat_mode Mode.Value.default in
      let cases, _ =
  ...
  | Pexp_field(srecord, lid) ->
      ...
      let mode =
        match label.lbl_global with
        | Global -> Mode.Value.default (* lbl_global==Global also implies Shared *)
  ...
  | Pexp_constraint (sarg, sty) ->
     (* Pretend separate = true, 1% slowdown for lablgtk *)
      ...
      let cty = Typetexp.transl_simple_type env ~fixed:false locality linearity sty in
  ...
  | Pexp_send (e, {txt=met}) ->
      if !Clflags.principal then begin_def ();
      let obj = type_exp env mode_default e in
  ...
  | Pexp_setinstvar (lab, snewval) -> begin
      ...
      | Mutable ->
          let newval =
            type_expect env mode_default snewval
        ...
        (path_self, _) ->
          let type_override (lab, snewval) =
            begin try
              ...
              (id, lab, type_expect env mode_default snewval (mk_expected (instance ty)))
            with
              Not_found ->
  ...
  | Pexp_poly(sbody, sty) ->
      ...
      let ty, cty =
        ...
        | Some sty ->
            ...
            let cty = Typetexp.transl_simple_type env ~fixed:false Global Many sty in
  ...
  | Pexp_letop{ let_ = slet; ands = sands; body = sbody } ->
      ...
      let arrow_desc = Nolabel, Mode.Alloc.default, Mode.Alloc.default in
      let ty_func =
      ...
      let cases, partial =
        type_cases Value body_env
          (simple_pat_mode Mode.Value.default)
          (mode_return Mode.Value.default)
  ...
  | Pexp_extension ({ txt = ("probe" | "ocaml.probe"); _ }, payload) ->
      ...
      | PStr
        ...
        let env = Env.add_locality_lock Mode.Locality.global env in
        Env.add_probe name;
        let exp = type_expect env mode_default arg
...
and type_binding_op_ident env s =
  ...
  submode ~env ~loc:lid.loc ~reason:Other mode mode_default;
  let path =
    match desc.val_kind with
    | Val_ivar _ ->
...
and type_function ?in_function loc attrs env (expected_mode : expected_mode)
  ...
  let env, region_locked =
    ...
    | None ->
      ...
      let env = Env.add_linearity_lock ~shared_context:Closure expected_mode.mode.linearity env in
      let env =
  ...
  if uncurried_function then begin
    ...
    begin
      match Mode.Locality.submode alloc_mode.locality ret_mode.locality with
    | Ok () -> ()
    | Error _e ->
        raise (Error(loc_fun, env, Uncurried_function_escapes))
    end;
    (* uniqueness are not constrained *)
    begin match Mode.Linearity.submode arg_mode.linearity ret_mode.linearity with
    | Ok () -> ()
    | Error _e ->
        raise (Error(loc_fun, env, Uncurried_function_linearity))
    end;
    begin match Mode.Linearity.submode alloc_mode.linearity ret_mode.linearity with
    | Ok () -> ()
    | Error _e ->
        raise (Error(loc_fun, env, Uncurried_function_linearity))
    end;

    (* in addition, unique arguments make the returning function once *)
    (* in other words, if argument <= unique, returning function >= once *)
    (* that is, returning function >= (dual of argument) *)
    begin match Mode.Linearity.submode (Mode.Linearity.from_dual arg_mode.uniqueness) ret_mode.linearity with
    | Ok () -> ()
    | Error _e ->
...
and type_apply_arg env ~app_loc ~funct ~index ~position ~partial_app (lbl, arg) =
  ...
  | Arg (Eliminated_optional_arg { ty_arg; _ }) ->
      ...
      (lbl, Arg (arg, Mode.Value.default))
...
and type_application env app_loc expected_mode position funct funct_mode sargs ret_tvar =
  ...
  | (* Special case for ignore: avoid discarding warning *)
    [Nolabel, sarg] when is_ignore funct ->
      ...
      let mode = mode_cross env ty_res (Mode.Value.of_alloc mres) in
  ...
  | _ ->
      ...
      let mode = mode_cross env ty_ret (Mode.Value.of_alloc mode_ret) in
...
and type_let
  ...
  let rec_mode_var =
    ...
    | Recursive -> Some Mode.Value.default
    | Nonrecursive -> None
  in
  let spatl =
    List.map
      (fun {pvb_pat=spat; pvb_expr=sexp; pvb_attributes=attrs} ->
         ...
         let pat_mode, exp_mode =
           if force_toplevel
           then simple_pat_mode Mode.Value.default, mode_default
...
and type_andops env sarg sands expected_ty =
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | [] ->
        type_expect env mode_default let_sarg
          (mk_expected expected_ty),
        []
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
        ...
        let exp =
          type_expect env mode_default sexp (mk_expected ty_arg)
        in
        begin try
...
and type_generic_array
  ...
  with_explanation explanation (fun () ->
    unify_exp_types loc env to_unify (generic_instance ty_expected));
  let argument_mode = expect_mode_cross env ty (mode_global expected_mode) in
  let argl =
    List.map
...
and type_comprehension_expr
  ...
  let comp_body =
    ...
    type_expect new_env mode_default sbody (mk_expected element_ty)
  in
  ...
  | In seq ->
      ...
      let pattern =
        ...
        type_pat
          ...
          ~alloc_mode:(simple_pat_mode Mode.Value.default)
          (ref env)
          pattern
          item_ty
      in
...
let maybe_check_uniqueness_exp exp =
  if Clflags.Extension.is_enabled Unique then
...
let type_expression env sexp =
  Typetexp.TyVarEnv.reset ();
  begin_def();
  let exp = type_exp env mode_default sexp in
  end_def();
...
let type_expect env e ty =
  let exp = type_expect env mode_default e ty in

  maybe_check_uniqueness_exp exp; exp
let type_exp env e =
  let exp = type_exp env mode_default e in
  maybe_check_uniqueness_exp exp; exp
let type_argument env e t1 t2 =
  let exp = type_argument env mode_default e t1 t2 in
  maybe_check_uniqueness_exp exp; exp
