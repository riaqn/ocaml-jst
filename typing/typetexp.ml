...
let transl_simple_type env ?univars ~fixed locality linearity styp =
...
let transl_simple_type_univars env styp =
  ...
  let typ, univs = TyVarEnv.collect_pre_univars begin fun () ->
    begin_def ();
    let typ = transl_type env TyVarEnv.univars_policy Mode.Locality.Global Mode.Linearity.Many styp in
...
let transl_simple_type_delayed env locality linearity styp =
  TyVarEnv.reset_locals ();
  begin_def ();
  let typ = transl_type env TyVarEnv.extensible_policy locality linearity styp in
  end_def ();
  make_fixed_univars typ.ctyp_type;
...
let transl_type_scheme env styp =
  ...
  | Ptyp_poly (vars, st) ->
     ...
     let typ = transl_simple_type env ~univars ~fixed:true Mode.Locality.Global Mode.Linearity.Many st in
     end_def();
     generalize typ.ctyp_type;
  ...
  | _ ->
     begin_def();
     let typ = transl_simple_type env ~fixed:false Mode.Locality.Global Mode.Linearity.Many styp in
     end_def();
     generalize typ.ctyp_type;
     typ
